...
if not os.path.exists('Face-Found'):
    print('Carpeta creada: Rostros encontrados')
    os.makedirs('Face-Found')
...
for imageName in imagesPathList:
    ...
    faces = faceCalssif.detectMultiScale(gray, 1.1, 5)

    for (x, y, w, h) in faces:
        cv2.rectangle(image, (x, y), (x+w, y+h), (128, 0, 255), 2)

    cv2.rectangle(image, (10, 5), (450, 25), (255, 255, 355), -1)
    cv2.putText(image, 'Presione s, para almacenar los rostros encontrados',
                (10, 20), 2, 0.5, (128, 0, 255), 1, cv2.LINE_AA)
    ...
    if k == ord('s'):
        for (x,y,w,h) in faces:
            ...
            cv2.imwrite('Face-Found/face_{}.jpg'.format(count), rostro)
            count += 1 
    elif k == 27:
        break
